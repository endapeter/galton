...
def simulate_and_visualize_trajectory_optimized(circles_data, circles_centers, grid_data, grid_counts, min_cx, min_cy, 
                                                cell_size, radius, x_init, y_init, vx_init, vy_init, e, h_init, rows, spacing, h_final, WALL_DISTANCE, g=9.81):
    ...
    save_path = os.path.join("figures", "random_verification_path.png")
...
def run_and_plot_galton_batch(n_balls, n_bins, generated_circles, circles_centers, grid_data, grid_counts, min_cx, min_cy,
    ...
    print(f"\n-> Trajectory landing positions explicitly saved to CSV file: '{csv_filename}'")
    ...
    if len(final_x_positions) > 0:
        ...
        # Save instead of show
        hist_save_path = os.path.join("figures", "galton_distribution.png")
        plt.savefig(hist_save_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"-> Histogram saved to {hist_save_path}")
        
    # Feature: Randomly select ONE configuration from the completed batch to visualize high-fidelity physics
